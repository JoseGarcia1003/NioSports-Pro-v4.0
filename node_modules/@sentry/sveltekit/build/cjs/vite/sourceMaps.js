...
async function makeCustomSentryVitePlugins(
  ...
  const adapterOutputDir = await svelteConfig.getAdapterOutputDir(svelteConfig$1, usedAdapter);
...
;
  ...
  const sentryPlugins = await vitePlugin.sentryVitePlugin(mergedOptions);
  ...
  const modifiedMainPlugin = { ...sentryMainPlugin, writeBundle: undefined };

  let isSSRBuild = true;

  const sourceMapSettingsPlugin = {
  ...
  const customDebugIdUploadPlugin = {
    ...
    closeBundle: async () => {
      ...
      for (const file of jsFiles) {
        try {
          await (sorcery ).load(file).then(async chain => {
            if (!chain) {
        ...
        try {
          ...
          const cleanedMapContent = mapContent.replace(
            ...
            new RegExp(core.escapeStringForRegex(utils.WRAPPED_MODULE_SUFFIX), 'gm'),
            '',
          );
 ...
 */
function _getUpdatedSourceMapSettings(
  viteConfig,
  sentryPluginOptions,
) {
...
function detectSentryRelease() {
  ...
  try {
    ...
    releaseFallback = core.uuid4();
  }
  const release = node.getSentryRelease() || releaseFallback;

  return release;
}

exports._getUpdatedSourceMapSettings = _getUpdatedSourceMapSettings;
