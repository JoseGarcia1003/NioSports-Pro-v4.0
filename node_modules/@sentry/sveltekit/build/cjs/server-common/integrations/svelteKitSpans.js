 ...
 * and data.
 */
function svelteKitSpansIntegration() {
  return {
 ...
 */
function _enhanceKitSpan(span) {
  let op = undefined;
  let origin = undefined;

  const spanName = span.description;

  const previousOp = span.op || span.data[core.SEMANTIC_ATTRIBUTE_SENTRY_OP];
  const previousOrigin = span.origin || span.data[core.SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN];
  ...
  if (!previousOp && op) {
    span.op = op;
    span.data[core.SEMANTIC_ATTRIBUTE_SENTRY_OP] = op;
  }

  if ((!previousOrigin || previousOrigin === 'manual') && origin) {
    span.origin = origin;
    span.data[core.SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN] = origin;
  }
}

exports._enhanceKitSpan = _enhanceKitSpan;
exports.svelteKitSpansIntegration = svelteKitSpansIntegration;
//# sourceMappingURL=svelteKitSpans.js.map
