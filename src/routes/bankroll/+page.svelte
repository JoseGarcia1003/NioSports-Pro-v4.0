...
<!--
  REEMPLAZA: renderBankrollView() de renders.js
  Gestión completa del bankroll: saldo actual, historial de movimientos,
  ROI acumulado y gráfico de evolución con Chart.js.
  
  PATRÓN: dbSubscribe en onMount → store → onDestroy cleanup.
  El gráfico se inicializa con afterUpdate cuando los datos están listos.
-->
<script>
  ...
  let showModal    = false;
  let modalType    = 'deposit';   // 'deposit' | 'withdraw' | 'reset'
  let modalAmount  = '';
  let modalNote    = '';
  ...
  onMount(async () => {
    ...
    // También suscribirse a los picks para calcular rendimiento
    cleanupPicks = dbSubscribe(
      userPath($userId, 'picks', 'totales'),
      (data) => {
        // Los picks se manejan en el store global — aquí solo
        // nos interesa disparar el re-cálculo de estadísticas
      }
    );
  });
  ...
  // Construir el gráfico cuando el canvas esté disponible y haya datos
  afterUpdate(() => {
  ...
  async function handleTransaction() {
    ...
    if (isNaN(amount) || amount <= 0) {
      toasts.error('Ingresa un monto válido mayor a 0.');
      return;
    }
    if (modalType === 'withdraw' && amount > current) {
      toasts.error('No puedes retirar más de tu saldo actual.');
      return;
    }
    ...
    try {
      ...
      const newBal    = modalType === 'deposit'
                        ? prevBal + amount
                        : prevBal - amount;
      ...
      const updates = {
        current: parseFloat(newBal.toFixed(2)),
        history: newHistory,
      };

      // Si es el primer depósito, también setear el bankroll inicial
      if (!snapshot.initial || snapshot.initial === 0) {
        updates.initial = parseFloat(newBal.toFixed(2));
      }

      await dbWrite(userPath($userId, 'bankroll'), updates);
      toasts.success(
        modalType === 'deposit'
          ? `✅ Depósito de $${amount.toFixed(2)} registrado.`
  ...
  async function handleReset() {
    const amount = parseFloat(modalAmount);
    if (isNaN(amount) || amount < 0) {
      toasts.error('Ingresa un monto válido.');
      return;
    }
    modalLoading = true;
    try {
      await dbWrite(userPath($userId, 'bankroll'), {
        ...
        initial:  parseFloat(amount.toFixed(2)),
        history:  [],
  ...
  function submit() {
    if (modalType === 'reset') handleReset();
    else handleTransaction();
  }
</script>

<!-- Chart.js CDN — solo se carga en esta ruta -->
<svelte:head>
  ...
  {:else}
    ...
    <div class="kpi-grid">
      <div class="kpi kpi--main">
        ...
        <div class="kpi__value kpi__value--accent">
          ${current.toFixed(2)}
        </div>
    ...
    <!-- ── Acciones ──────────────────────────────────────── -->
    <div class="actions">
      <button class="btn btn--deposit" on:click={() => openModal('deposit')}>
        + Depósito
      </button>
      <button class="btn btn--withdraw" on:click={() => openModal('withdraw')}
        disabled={current <= 0}>
        − Retiro
      </button>
      <button class="btn btn--reset" on:click={() => openModal('reset')}>
        ↺ Reiniciar
      </button>
    </div>
          ...
          {#each [...history].reverse() as entry, i (entry.date + i)}
            <div class="history-item">
              ...
              <div class="history-item__info">
                ...
                {#if entry.note}
                  <span class="history-item__note">{entry.note}</span>
                {/if}
              ...
              <div class="history-item__right">
                ...
                <span class="history-item__balance">
    ...
    <div class="modal">
      <div class="modal__header">
        <h3 class="modal__title">
          ...
          : modalType === 'withdraw' ? '− Registrar retiro'
          : '↺ Reiniciar bankroll'}
        </h3>
      ...
      {#if modalType === 'reset'}
        <p class="modal__warning">
          ⚠️ Esto borrará todo el historial y establecerá un nuevo capital inicial.
        </p>
      {/if}

      <label class="field">
        <span class="field__label">
          {modalType === 'reset' ? 'Nuevo capital inicial ($)' : 'Monto ($)'}
        </span>
        <input
          class="field__input"
          type="number"
      ...
      <div class="modal__actions">
        <button class="btn btn--ghost" on:click={closeModal} disabled={modalLoading}>
          Cancelar
        </button>
        <button
          class="btn"
...
<style>
  ...
  /* ── Skeleton ───────────────────────────────────────────────── */
  .skeleton-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(170px, 1fr)); gap: 14px; margin-bottom: 24px; }
