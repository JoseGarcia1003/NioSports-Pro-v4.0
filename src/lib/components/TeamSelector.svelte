...
<!-- Custom dropdown for team selection — replaces native <select> -->
<script>
  ...
  /** @type {string} Currently selected team */
  export let value = '';

  /** @type {string} Placeholder text */
  export let placeholder = 'Seleccionar equipo...';

  /** @type {string[]} Teams to disable (e.g. already selected by the other selector) */
  export let disabled = [];

  /** @type {string} Accent color for the border/focus */
...
<div class="dd-container" bind:this={containerEl} on:keydown={handleKeydown} style="--accent: {accent}">
  ...
  <button class="dd__trigger" class:dd__trigger--open={open} class:dd__trigger--selected={!!value} on:click={toggle} type="button">
    {#if value}
      ...
      <button class="dd__clear" on:click={clear} type="button" aria-label="Limpiar selección">
        <X size={14} />
      </button>
    {:else}
      ...
      <div class="dd__list">
        ...
        {:else}
          {#each filtered as team, i}
            ...
            <div
              class="dd__item"
              class:dd__item--disabled={isDisabled}
              class:dd__item--selected={isSelected}
              class:dd__item--highlighted={isHighlighted}
              on:click={() => select(team)}
              role="option"
            ...
            >
              ...
              {#if isSelected}
                <span class="dd__item-check">✓</span>
              {/if}
              {#if isDisabled}
                <span class="dd__item-tag">Ya seleccionado</span>
              {/if}
            </div>
          {/each}
        {/if}
...
<style>
  ...
  .dd__item:hover:not(.dd__item--disabled) {
    background: rgba(99, 102, 241, 0.1);
    color: #fff;
  }

  .dd__item--highlighted:not(.dd__item--disabled) {
